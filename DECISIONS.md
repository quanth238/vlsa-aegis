...
time-conditioned weights so every q_hat is generated by a temporally coherent
displacement trajectory. Preserve full Cartesian actions, complete controller
state, structured orientation, grouped episodes, known FK, and analytic
ellipsoid safety.

Run the existing-data mechanism gate before spending simulation on new
episodes. E01/E11/E21/E31/E41 are now immutable final reservations selected by
index before their new safety labels are inspected. A mechanism failure blocks
