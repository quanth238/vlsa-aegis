...
class Pi0(_model.BaseModel):
    ...
    def sample_actions_with_flow_guidance(
        self,
        rng: at.KeyArrayLike,
        observation: _model.Observation,
        *,
        flow_guidance_rows: jax.Array,
        flow_guidance_lower: jax.Array,
        num_steps: int | at.Int[at.Array, ""] = 10,
        noise: at.Float[at.Array, "b ah ad"] | None = None,
    ) -> _model.Actions:
        """Opt-in pi0.5 sampling with projection after every Euler update."""
        ...
        if noise is None:
            noise = jax.random.normal(rng, (batch_size, self.action_horizon, self.action_dim))
        rows = jnp.asarray(flow_guidance_rows, dtype=noise.dtype)
        lower = jnp.asarray(flow_guidance_lower, dtype=noise.dtype)
    ...
    def sample_actions_with_repulsive_flow_guidance(
        self,
        rng: at.KeyArrayLike,
        observation: _model.Observation,
        *,
        ...
        repulsive_euler_mask: jax.Array,
        repulsive_step_size: float,
        repulsive_model_lower: jax.Array,
        repulsive_model_upper: jax.Array,
        num_steps: int | at.Int[at.Array, ""] = 10,
        noise: at.Float[at.Array, "b ah ad"] | None = None,
    ) -> _model.Actions:
        """Add bounded fixed physical repulsion during final Euler updates."""
        ...
        euler_mask = jnp.asarray(repulsive_euler_mask, dtype=noise.dtype)
        ...
        if slot_mask.shape != (self.action_horizon, 1):
            raise ValueError("repulsive-flow slot mask shape differs")
        if euler_mask.shape != (10,):
            raise ValueError("repulsive-flow Euler mask shape differs")
        ...
        def step(carry):
            ...
            x_next = x_t + dt * v_t
            strength = (
                jnp.asarray(repulsive_step_size, dtype=noise.dtype)
                * euler_mask[step_index]
            )
        ...
        x_0, _, _ = jax.lax.while_loop(
            cond,
            step,
            (noise, jnp.asarray(1.0, dtype=noise.dtype), jnp.asarray(0, dtype=jnp.int32)),
        )
        return x_0
