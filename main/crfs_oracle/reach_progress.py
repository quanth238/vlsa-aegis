...
def _distance(left: Sequence[float], right: Sequence[float]) -> float:
    ...
    # Accumulate explicitly so reconstruction is identical before and after
    # Python 3.12 changed the implementation of ``sum`` for floats.
    squared_distance = 0.0
    for a, b in zip(left, right):
        squared_distance += (float(a) - float(b)) ** 2
