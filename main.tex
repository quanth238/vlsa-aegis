...
The mechanism study uses Eq.~\eqref{eq:clearance} at every physics substep in MuJoCo \cite{todorov2012mujoco}: the EEF-sphere center and each oriented obstacle box are read from simulator transforms, and their exact primitive signed distance is evaluated. Physical end-effector--obstacle contacts provide a one-way conservatism check: a contact at positive proxy clearance is forbidden, while negative conservative clearance before physical contact is expected. Raw \texttt{mj\_geomDistance} queries involving the released gripper collision mesh are retained only for diagnostics because the H03 audit reproduced false negative mesh--box distances without contact, consistent with the upstream open distance-query issue \cite{mujocoissue1784}. This metric is more informative than the obstacle-displacement flag used in the released benchmark runner, which marks a collision after the obstacle position changes by more than a fixed threshold \cite{vlsarepo}. The original benchmark metric should still be reported later for comparability, but it should not be the supervision source for the proposed method.
...
\begin{enumerate}[leftmargin=*,nosep]
    ...
    \item Calibrate the sphere--box zero-clearance boundary against reported first contact within the simulator's numerical tolerance.
...
\begin{enumerate}[leftmargin=*,nosep]
    ...
    \item \textbf{Branch and measure.} From a saved simulator state, execute nominal, random, oracle, and learned actions separately. Track the conservative EEF-sphere--primitive-obstacle signed distance from simulator transforms at every physics substep, and retain physical contacts as a cross-check. The released runner already exposes a five-step replanning parameter and a safe-success log, but its displacement-based collision flag should not be used as the continuous supervision target \cite{vlsarepo}.
\end{enumerate}
