#!/usr/bin/env python3
"""Collect one adaptive post-AEGIS L5 boundary state and exact labels."""
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    _require(
        ...
        "adaptive base method file differs",
    )
    ...
    _require(coverage["retained_state_count"] == 1,
             "adaptive collector requires one retained state")
    ...
    def definitions(nominal, frame, base):
        return coarse_candidate_definitions(
            nominal, frame, base, adaptive
        )
    ...
    print(json.dumps({
        ...
        "bracket_found": result["adaptive_boundary_sampling"]["bracket_found"],
        "coarse_candidates": len(result["adaptive_boundary_sampling"][
