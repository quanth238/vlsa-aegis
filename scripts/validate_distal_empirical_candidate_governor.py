#!/usr/bin/env python3
"""Independently validate the empirical candidate-governor oracle."""
...
def validate(
    *, result_path: Path, config_path: Path, expected_commit: str
    ...
    if result.get("schema_version") != RESULT_SCHEMA:
        raise ValueError("empirical candidate-governor result schema differs")
    if result.get("source", {}).get("commit") != expected_commit:
        raise ValueError("empirical candidate-governor producer commit differs")
    payload = result.get("result_payload_sha256")
    copy = dict(result)
    copy.pop("result_payload_sha256", None)
    if payload != _sha256(_canonical(copy)):
        raise ValueError("empirical candidate-governor result payload differs")
    if result.get("config") != config:
        raise ValueError("empirical candidate-governor embedded config differs")
    for case_config, case_result in zip(config["cases"], result["cases"]):
        if _file_sha256(Path(case_config["source_result"])) != case_config[
            ...
            raise ValueError("empirical candidate-governor source file differs")
        if case_result["case_id"] != case_config["case_id"]:
            raise ValueError("empirical candidate-governor case order differs")
        if int(case_result["state_step"]) != int(case_config["state_step"]):
            raise ValueError("empirical candidate-governor state order differs")
    classification = classify(result["cases"], config["gate"])
    if result["classification"] != classification:
        raise ValueError("empirical candidate-governor classification differs")
    return {
        "schema_version": "vlsa_distal_empirical_candidate_governor_validation.v1",
        "status": "complete",
