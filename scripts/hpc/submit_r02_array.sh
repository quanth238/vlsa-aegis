...
usage='usage: RUN_ID=... submit_r02_array.sh MANIFEST_JSONL CONFIG_JSON R01_RAW_ROOT_REMOTE R01_SUMMARY_REMOTE R01_SUMMARY_SHA256 PARITY_ARTIFACT_REMOTE PARITY_ARTIFACT_SHA256 [CONCURRENCY]'
...
case "$CONCURRENCY" in
  1|2) ;;
  *) echo "R02 full-H100 array concurrency must be 1 or 2" >&2; exit 2 ;;
esac
: "${RUN_ID:?set RUN_ID to an immutable exact R02 array identifier}"
...
test "$COUNT" -eq 20 || { echo "R02 array requires the frozen original 20-case manifest" >&2; exit 2; }
LAST=$((COUNT - 1))
...
ssh "$HOST" bash -s -- \
  ...
  "$PARITY_ARTIFACT" "$PARITY_ARTIFACT_SHA256" "$LAST" "$CONCURRENCY" <<'REMOTE'
set -euo pipefail
remote_repo=$1
run_id=$2
manifest=$3
...
test -f "$remote_repo/slurm/r02_main_array.sbatch"
