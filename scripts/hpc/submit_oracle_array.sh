...
ssh "$HOST" "mkdir -p /mnt/data/quanth/slurm_logs/crfs-oracle && cd '$REMOTE_REPO' && RUN_ID='$RUN_ID' MANIFEST='$MANIFEST' REMOTE_REPO='$REMOTE_REPO' sbatch --array='0-$LAST%$CONCURRENCY' --output='/mnt/data/quanth/slurm_logs/crfs-oracle/%x-%A_%a.out' slurm/oracle_main_array.sbatch"
