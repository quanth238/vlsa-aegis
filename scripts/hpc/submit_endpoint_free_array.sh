...
test "$COUNT" -eq 20 || { echo "endpoint-free array requires the frozen 20-case evaluation manifest" >&2; exit 2; }
LAST=$((COUNT - 1))
...
if pathlib.Path(config.get("manifest", "")).name != pathlib.Path(sys.argv[2]).name:
    raise SystemExit("array manifest does not match the endpoint-free config")
PY
