...
usage='usage: RUN_ID=... EXPECTED_RELEASE_COMMIT=... submit_r05a_reference_trajectory_lift_canary.sh'
: "${RUN_ID:?$usage}"
: "${EXPECTED_RELEASE_COMMIT:?$usage}"
case "$RUN_ID" in *[!A-Za-z0-9._-]*|'') echo "unsafe TRL-00A run id" >&2; exit 2 ;; esac
case "$RUN_ID" in [A-Za-z0-9]*) ;; *) echo "TRL-00A run id must start alphanumeric" >&2; exit 2 ;; esac
case "$EXPECTED_RELEASE_COMMIT" in *[!0-9a-f]*|'') echo "invalid TRL-00A release commit" >&2; exit 2 ;; esac
test "${#EXPECTED_RELEASE_COMMIT}" = 40 || { echo "TRL-00A release commit must have 40 hex characters" >&2; exit 2; }

ROOT=$(CDPATH= cd -- "$(dirname -- "$0")/../.." && pwd)
cd "$ROOT"
HOST=${VINUNI_HOST:-vinuni}
REMOTE_REPO=${REMOTE_REPO:-/home/quanth/working_space/vlsa-aegis-crfs}
CONFIG=configs/experiments/r05a_reference_trajectory_lift_canary.json
PREREG_ADR=docs/decisions/0060-preregister-reference-trajectory-lift-canary.md
...
for path in "$CONFIG" "$PREREG_ADR" \
  ...
  scripts/hpc/run_r05a_reference_trajectory_lift_workload.sh \
  scripts/hpc/run_r05a_reference_trajectory_lift_canary.sh \
  scripts/hpc/validate_r05a_reference_trajectory_lift_canary.sh \
  scripts/hpc/submit_r05a_reference_trajectory_lift_canary.sh \
  slurm/r05a_reference_trajectory_lift_canary_h100.sbatch \
  slurm/r05a_reference_trajectory_lift_canary_validate_cpu.sbatch; do
  test -f "$path" && test ! -L "$path" || { echo "missing or symlinked TRL-00A source: $path" >&2; exit 2; }
done
jq -e '
...
' "$CONFIG" >/dev/null || {
  echo "TRL-00A is implemented but not released for H100 submission" >&2
  exit 2
}
RELEASE_ADR=$(jq -er '.execution_release.decision_artifact' "$CONFIG")
case "$RELEASE_ADR" in
  docs/decisions/*-release-reference-trajectory-lift-canary.md) ;;
  *) echo "TRL-00A release decision path is not the registered release ADR" >&2; exit 2 ;;
esac
test -f "$RELEASE_ADR" && test ! -L "$RELEASE_ADR" || {
  echo "missing or symlinked TRL-00A release artifact: $RELEASE_ADR" >&2; exit 2
}
test -z "$(git status --porcelain)" || { echo "TRL-00A submission requires a clean reviewed tree" >&2; exit 2; }
test "$(git rev-parse HEAD)" = "$EXPECTED_RELEASE_COMMIT" || { echo "local TRL-00A HEAD differs from release" >&2; exit 2; }
test "$(git rev-parse refs/remotes/origin/agent/crfs-oracle-harness)" = "$EXPECTED_RELEASE_COMMIT" || { echo "origin TRL-00A release ref differs" >&2; exit 2; }
...
test "$REGISTERED_RUN_ID" = "$RUN_ID" || { echo "TRL-00A caller run id differs from release" >&2; exit 2; }
test "$(git rev-list --parents -n 1 "$EXPECTED_RELEASE_COMMIT")" = "$EXPECTED_RELEASE_COMMIT $ACCEPTED_IMPLEMENTATION_COMMIT" || { echo "TRL-00A release is not the direct implementation child" >&2; exit 2; }
jq -e --arg run "$RUN_ID" --arg implementation "$ACCEPTED_IMPLEMENTATION_COMMIT" --arg release "$RELEASE_ADR" '
  ...
  and .execution_release.resources == {
    ...
    host_memory_mib:65536,time_limit:"00:30:00",array:"0-0%1",requeue:false,
...
' "$CONFIG" >/dev/null || { echo "TRL-00A exact release contract changed" >&2; exit 2; }

PARENT_CONFIG=$(git show "$ACCEPTED_IMPLEMENTATION_COMMIT:$CONFIG")
jq -e '.ready_to_run == false and (.blocked_on|length)>0 and .preregistration.h100_submission_authorized == false and .execution_release == null' <<<"$PARENT_CONFIG" >/dev/null || { echo "TRL-00A implementation parent was not fail-closed" >&2; exit 2; }
test "$(jq -cS 'del(.config_status,.ready_to_run,.blocked_on,.preregistration.h100_submission_authorized,.execution_release)' <<<"$PARENT_CONFIG")" = \
  ...
  echo "TRL-00A release changed scientific content" >&2; exit 2
}
...
test "$(printf '%s\n' "$RELEASE_DIFF" | LC_ALL=C sort)" = "$EXPECTED_RELEASE_DIFF" || {
  echo "TRL-00A release diff must contain exactly the config and registered release ADR" >&2; exit 2
}

# This local command performs shell-only control-plane inspection.  It does
# not require a currently free H100; a healthy busy worker may queue TRL-00A.
scripts/hpc/preflight.sh

ssh "$HOST" bash -s -- "$REMOTE_REPO" "$RUN_ID" "$EXPECTED_RELEASE_COMMIT" "$ACCEPTED_IMPLEMENTATION_COMMIT" "$RELEASE_ADR" <<'REMOTE'
set -euo pipefail
remote_repo=$1
run_id=$2
expected_commit=$3
...
gpu_slurm=$remote_repo/slurm/r05a_reference_trajectory_lift_canary_h100.sbatch
cpu_slurm=$remote_repo/slurm/r05a_reference_trajectory_lift_canary_validate_cpu.sbatch
...
validate_gpu_records() {
  ...
  crfs_validate_exact_array_task_identity "$task_record" "$gpu_id" 0 || {
    echo "held TRL-00A GPU task identity changed" >&2; return 1
  }
  for field in \
    "ArrayJobId=$gpu_id" "ArrayTaskId=0" \
    "JobState=PENDING" "Reason=JobHeldUser" "ReqNodeList=worker-1" \
    "Partition=main" "Account=normal" "QOS=normal" "TimeLimit=00:30:00" \
    "Requeue=0" "NumNodes=1" "NumCPUs=8" "CPUs/Task=8"; do
    require_job_field "$task_record" "$field" "held TRL-00A GPU task"
  done
  require_job_field "$parent_record" "ArrayJobId=$gpu_id" "held TRL-00A GPU parent"
  array_task_id=$(printf '%s\n' "$parent_record" | sed -n 's/.* ArrayTaskId=\([^ ]*\).*/\1/p')
  case "$array_task_id" in 0|0%1) ;; *) echo "held TRL-00A GPU parent ArrayTaskId changed: $array_task_id" >&2; return 1 ;; esac
  require_job_field "$parent_record" "ArrayTaskThrottle=1" "held TRL-00A GPU parent"
  require_single_tres "$task_record" cpu 8 "held TRL-00A GPU task"
  require_single_memory_tres "$task_record" 65536 "held TRL-00A GPU task"
  require_single_tres "$task_record" node 1 "held TRL-00A GPU task"
  require_single_tres "$task_record" gres/gpu 1 "held TRL-00A GPU task"
  require_exact_tres_keys "$task_record" "$(printf '%s\n' cpu gres/gpu mem node | LC_ALL=C sort)" "held TRL-00A GPU task"
  test "$(printf '%s\n' "$task_record" | sed -n 's/.* ReqTRES=\([^ ]*\).*/\1/p' | tr ',' '\n' | awk -F= '$1 ~ /^gres\/gpu/ {print}')" = "gres/gpu=1" || {
    echo "held TRL-00A GPU task has non-exact GPU ReqTRES" >&2; return 1
  }
}
...
validate_cpu_record() {
  ...
  for field in \
    ...
    require_job_field "$record" "$field" "TRL-00A CPU publisher"
  done
  case " $record " in
    *" ArrayJobId="*|*" ArrayTaskId="*|*" ArrayTaskThrottle="*)
      echo "TRL-00A CPU publisher unexpectedly became an array" >&2; return 1 ;;
  esac
  require_single_tres "$record" cpu 2 "TRL-00A CPU publisher"
  require_single_memory_tres "$record" 8192 "TRL-00A CPU publisher"
  require_single_tres "$record" node 1 "TRL-00A CPU publisher"
  require_exact_tres_keys "$record" "$(printf '%s\n' cpu mem node | LC_ALL=C sort)" "TRL-00A CPU publisher"
  if printf '%s\n' "$record" | sed -n 's/.* ReqTRES=\([^ ]*\).*/\1/p' | tr ',' '\n' | awk -F= '$1 ~ /^gres\/gpu/ {found=1} END {exit !found}'; then
    echo "TRL-00A CPU publisher unexpectedly requests a GPU" >&2; return 1
  fi
  case " $record " in *" TRESPerNode=gres/gpu:"*) echo "TRL-00A CPU publisher has per-node GPU resources" >&2; return 1 ;; esac
  ...
  test "$observed_dependency" = "afterany:$gpu_id" || {
    echo "TRL-00A CPU dependency changed: $observed_dependency" >&2; return 1
  }
}
...
bound_paths=(
  ...
  docs/decisions/0060-preregister-reference-trajectory-lift-canary.md
  "$release_adr_relative"
  ...
  main/run_crfs_r05a_reference_trajectory_lift_canary.py
  main/publish_crfs_r05a_reference_trajectory_lift_canary.py
  schemas/r05a-reference-trajectory-lift-canary-envelope.schema.json
  schemas/r05a-reference-trajectory-lift-canary-publication-receipt.schema.json
  tests/test_r05a_reference_trajectory_lift_preregistration.py
  tests/test_r05a_reference_trajectory_lift_canary.py
  tests/test_r05a_reference_trajectory_lift_validation.py
  tests/test_reference_trajectory_lift.py
  ...
  scripts/hpc/submit_r05a_reference_trajectory_lift_canary.sh
  slurm/r05a_reference_trajectory_lift_canary_h100.sbatch
  slurm/r05a_reference_trajectory_lift_canary_validate_cpu.sbatch
)
for relative in "${bound_paths[@]}"; do
  test -f "$remote_repo/$relative" && test ! -L "$remote_repo/$relative" || { echo "missing remote TRL-00A source: $relative" >&2; exit 2; }
done
. "$array_identity_helper"
test "$(git -C "$remote_repo" rev-parse HEAD)" = "$expected_commit" || { echo "remote TRL-00A commit mismatch" >&2; exit 2; }
test -z "$(git -C "$remote_repo" status --porcelain)" || { echo "remote TRL-00A tree is dirty" >&2; exit 2; }
test "$(git -C "$remote_repo" rev-parse refs/remotes/origin/agent/crfs-oracle-harness)" = "$expected_commit" || { echo "remote origin TRL-00A release differs" >&2; exit 2; }
test "$(git -C "$remote_repo" rev-list --parents -n 1 "$expected_commit")" = "$expected_commit $accepted_implementation" || { echo "remote TRL-00A release parent differs" >&2; exit 2; }
...
expected_remote_release_diff=$(printf '%s\n' \
  configs/experiments/r05a_reference_trajectory_lift_canary.json \
  "$release_adr_relative" | LC_ALL=C sort)
test "$remote_release_diff" = "$expected_remote_release_diff" || { echo "remote TRL-00A release diff is not the exact two-path release" >&2; exit 2; }
test -f "$release_adr" && test ! -L "$release_adr" || { echo "remote TRL-00A release ADR is missing or symlinked" >&2; exit 2; }
test "$(jq -er '.execution_release.run_id' "$config")" = "$run_id" || { echo "remote TRL-00A run id changed" >&2; exit 2; }
test "$(jq -er '.execution_release.accepted_implementation_commit' "$config")" = "$accepted_implementation" || { echo "remote TRL-00A implementation changed" >&2; exit 2; }
...
test -f "$checkpoint" && test ! -L "$checkpoint" && test -r "$checkpoint" || { echo "canonical TRL-00A checkpoint is missing, symlinked, or unreadable" >&2; exit 2; }
...
for path in "$checkpoint_config" "$normalization_asset"; do
  test -f "$path" && test ! -L "$path" || { echo "missing or symlinked TRL-00A checkpoint metadata: $path" >&2; exit 2; }
done
...
test -z "$inherited_sbatch_options" || { echo "inherited SBATCH options could change TRL-00A: $inherited_sbatch_options" >&2; exit 2; }
test ! -e "$run_root" || { echo "immutable TRL-00A run id is already used" >&2; exit 2; }
if [ -n "$(squeue -h -u "$(whoami)" -o '%i')" ]; then echo "TRL-00A not submitted: user queue is not empty" >&2; exit 2; fi
...
gpu_submission=$(RUN_ID="$run_id" EXPECTED_GIT_COMMIT="$expected_commit" REMOTE_REPO="$remote_repo" \
  ...
  --cpus-per-task=8 --mem=64G --time=00:30:00 --no-requeue --nodelist=worker-1 \
  --array=0-0%1 --output='/mnt/data/quanth/slurm_logs/crfs-oracle/%x-%A_%a.out' "$gpu_slurm")
gpu_job_id=${gpu_submission%%;*}
case "$gpu_job_id" in *[!0-9]*|'') echo "invalid TRL-00A GPU job id" >&2; exit 2 ;; esac
...
jq -n --arg run "$run_id" --arg commit "$expected_commit" --arg gpu "$gpu_job_id" --arg now "$(date -u +%Y-%m-%dT%H:%M:%SZ)" --argjson resources "$resources" \
  '{schema_version:"1.0",artifact_role:"r05a_reference_trajectory_lift_canary_held_gpu_submission",status:"sbatch_returned_held_gpu_id",run_id:$run,git_commit:$commit,gpu_slurm_array_job_id:$gpu,gpu_slurm_array_task_id:0,exact_gpu_task_id:($gpu+"_0"),source_node:"worker-1",resources:$resources,released_at_receipt_time:false,scientific_claim_allowed:false,simulator_efficacy_claim_allowed:false,infeasibility_claim_allowed:false,probe_training_authorized:false,timestamp_utc:$now}' >"$held_tmp"
...
while IFS= read -r relative; do
  ...
  digest=$(sha256sum "$remote_repo/$relative" | awk '{print $1}')
  repository_hashes=$(jq -c --arg key "$relative" --arg value "$digest" '. + {($key):$value}' <<<"$repository_hashes")
done < <(git -C "$remote_repo" ls-files 'tests/test*reference_trajectory_lift*.py' 'schemas/r05a-reference-trajectory-lift-canary*.json')
...
case "$cpu_job_id" in *[!0-9]*|'') echo "invalid TRL-00A CPU job id" >&2; exit 2 ;; esac
...
jq -e --arg run "$run_id" --arg commit "$expected_commit" --arg gpu "$gpu_job_id" --arg cpu "$cpu_job_id" \
  ...
  ' "$release_fingerprint" >/dev/null || { echo "TRL-00A final pre-release fingerprint changed" >&2; exit 2; }
while IFS='|' read -r key path; do
  ...
  test "$expected_path" = "$path" || { echo "TRL-00A fingerprint path changed: $key" >&2; exit 2; }
  test -f "$path" && test ! -L "$path" || { echo "TRL-00A fingerprint input missing or symlinked: $key" >&2; exit 2; }
  test "$(sha256sum "$path" | awk '{print $1}')" = "$expected_sha" || { echo "TRL-00A fingerprint digest changed: $key" >&2; exit 2; }
done <<EOF
...
EOF
test "$(cat "$release_fingerprint_sha_file")" = "$release_fingerprint_sha" || { echo "TRL-00A fingerprint digest sidecar changed" >&2; exit 2; }
test "$(wc -l <"$release_fingerprint_sha_file" | tr -d ' ')" = 1 || { echo "TRL-00A fingerprint digest sidecar is not one line" >&2; exit 2; }
...
for relative in "${bound_paths[@]}"; do
  test "$(sha256sum "$remote_repo/$relative" | awk '{print $1}')" = "$(jq -er --arg path "$relative" '.repository_file_sha256[$path]' "$source_contract")" || {
    echo "TRL-00A final source fingerprint changed: $relative" >&2; exit 2
  }
done
