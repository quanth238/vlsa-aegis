...
# Control-plane guard for the exact held TRL-00A allocation.  The actual model
...
WORKLOAD=$REMOTE_REPO/scripts/hpc/run_r05a_reference_trajectory_lift_workload.sh
...
case "$SLURM_ARRAY_JOB_ID" in *[!0-9]*|'') echo "invalid TRL-00A array id" >&2; exit 2 ;; esac
test "$SLURM_ARRAY_TASK_ID" = 0 || { echo "TRL-00A is fixed to task zero" >&2; exit 2; }
test "$(hostname -s)" = worker-1 || { echo "TRL-00A is source-pinned to worker-1" >&2; exit 2; }
test -f "$CONFIG" && test ! -L "$CONFIG" || { echo "missing released TRL-00A config" >&2; exit 2; }
test -x "$WORKLOAD" || { echo "missing TRL-00A workload" >&2; exit 2; }
test -f "$ARRAY_IDENTITY_HELPER" && test ! -L "$ARRAY_IDENTITY_HELPER" || { echo "missing TRL-00A array identity helper" >&2; exit 2; }
...
test "$SUBMISSION" = "$RUN_ROOT/submission.json" || { echo "TRL-00A submission path changed" >&2; exit 2; }
...
for path in "$SOURCE_CONTRACT" "$HELD_GPU_SUBMISSION" "$SUBMISSION" "$RELEASE_FINGERPRINT" "$RELEASE_FINGERPRINT_SHA256_FILE"; do
  test -f "$path" && test ! -L "$path" || { echo "missing or symlinked TRL-00A transaction artifact: $path" >&2; exit 2; }
done
...
test "$(sha256sum "$SOURCE_CONTRACT" | awk '{print $1}')" = "$EXPECTED_SOURCE_CONTRACT_SHA256" || {
  echo "TRL-00A source-contract digest changed" >&2; exit 2
}
...
test "$(sha256sum "$SUBMISSION" | awk '{print $1}')" = "$EXPECTED_SUBMISSION_SHA256" || { echo "TRL-00A submission receipt changed" >&2; exit 2; }

CPU_PUBLISHER_JOB_ID=$(jq -er '.cpu_afterany_job_id' "$RELEASE_FINGERPRINT")
case "$CPU_PUBLISHER_JOB_ID" in *[!0-9]*|'') echo "invalid TRL-00A CPU publisher identity" >&2; exit 2 ;; esac
jq -e --arg run "$RUN_ID" --arg commit "$EXPECTED_GIT_COMMIT" --arg gpu "$SLURM_ARRAY_JOB_ID" --arg cpu "$CPU_PUBLISHER_JOB_ID" \
  ...
  ' "$RELEASE_FINGERPRINT" >/dev/null || { echo "TRL-00A release fingerprint differs from this allocation" >&2; exit 2; }
jq -e --arg run "$RUN_ID" --arg commit "$EXPECTED_GIT_COMMIT" --arg gpu "$SLURM_ARRAY_JOB_ID" --arg cpu "$CPU_PUBLISHER_JOB_ID" \
  --arg source "$SOURCE_CONTRACT" --arg source_sha "$EXPECTED_SOURCE_CONTRACT_SHA256" \
  --arg held "$HELD_GPU_SUBMISSION" --arg held_sha "$EXPECTED_HELD_GPU_SUBMISSION_SHA256" \
  ...
  ' "$SUBMISSION" >/dev/null || { echo "TRL-00A submission receipt differs from this allocation" >&2; exit 2; }
while IFS='|' read -r key; do
  ...
  test -f "$path" && test ! -L "$path" || { echo "TRL-00A fingerprint binding is missing: $key" >&2; exit 2; }
  test "$(sha256sum "$path" | awk '{print $1}')" = "$digest" || { echo "TRL-00A fingerprint binding changed: $key" >&2; exit 2; }
done <<'EOF'
...
test "$(git -C "$REMOTE_REPO" rev-parse HEAD)" = "$EXPECTED_GIT_COMMIT" || {
  echo "TRL-00A remote HEAD differs from release" >&2; exit 2
}
test -z "$(git -C "$REMOTE_REPO" status --porcelain)" || {
  echo "TRL-00A requires a clean remote source tree" >&2; exit 2
}
...
case "$RELEASE_ADR" in
  ...
  *) echo "TRL-00A execution release decision path changed" >&2; exit 2 ;;
esac
...
' "$CONFIG" >/dev/null || { echo "TRL-00A execution release changed" >&2; exit 2; }
test "$(git -C "$REMOTE_REPO" rev-list --parents -n 1 "$EXPECTED_GIT_COMMIT")" = \
  "$EXPECTED_GIT_COMMIT $ACCEPTED_IMPLEMENTATION_COMMIT" || {
  echo "TRL-00A release is not the direct child of its implementation" >&2; exit 2
}
test "$(git -C "$REMOTE_REPO" diff --name-only "$ACCEPTED_IMPLEMENTATION_COMMIT" "$EXPECTED_GIT_COMMIT" | LC_ALL=C sort)" = \
  ...
  echo "TRL-00A release diff is not the exact config-plus-ADR release" >&2; exit 2
}
...
crfs_validate_exact_array_task_identity "$job_record" "$SLURM_ARRAY_JOB_ID" 0 || {
  echo "TRL-00A exact array task identity changed" >&2; exit 2
}
for field in "ArrayJobId=$SLURM_ARRAY_JOB_ID" "ArrayTaskId=0" JobState=RUNNING Partition=main Account=normal QOS=normal TimeLimit=00:30:00 Requeue=0 ReqNodeList=worker-1 NumNodes=1 NumCPUs=8 CPUs/Task=8; do
  case " $job_record " in *" $field "*) ;; *) echo "TRL-00A job field changed: $field" >&2; exit 2 ;; esac
done
req_tres=$(printf '%s\n' "$job_record" | sed -n 's/.* ReqTRES=\([^ ]*\).*/\1/p')
test "$(printf '%s\n' "$req_tres" | tr ',' '\n' | awk -F= 'NF && $1!="billing" {print $1}' | LC_ALL=C sort)" = "$(printf '%s\n' cpu gres/gpu mem node | LC_ALL=C sort)" || { echo "TRL-00A ReqTRES key set changed" >&2; exit 2; }
test "$(printf '%s\n' "$req_tres" | tr ',' '\n' | awk -F= '$1=="cpu" {print $2}')" = 8 || { echo "TRL-00A CPU request changed" >&2; exit 2; }
case "$(printf '%s\n' "$req_tres" | tr ',' '\n' | sed -n 's/^mem=//p')" in 64G|65536M) ;; *) echo "TRL-00A memory request changed" >&2; exit 2 ;; esac
test "$(printf '%s\n' "$req_tres" | tr ',' '\n' | awk -F= '$1=="node" {print $2}')" = 1 || { echo "TRL-00A node request changed" >&2; exit 2; }
test "$(printf '%s\n' "$req_tres" | tr ',' '\n' | awk -F= '$1=="gres/gpu" {print $2}')" = 1 || { echo "TRL-00A GPU request changed" >&2; exit 2; }
test "$(printf '%s\n' "$req_tres" | tr ',' '\n' | awk -F= '$1 ~ /^gres\/gpu/ {print}')" = gres/gpu=1 || { echo "TRL-00A exact GPU request changed" >&2; exit 2; }

jq -e --arg run "$RUN_ID" --arg commit "$EXPECTED_GIT_COMMIT" --arg gpu "$SLURM_ARRAY_JOB_ID" '
  .schema_version == "1.0"
  and .artifact_role == "r05a_reference_trajectory_lift_canary_source_contract"
...
' "$SOURCE_CONTRACT" >/dev/null || { echo "TRL-00A source contract differs from allocation" >&2; exit 2; }
...
for relative in \
  ...
  scripts/hpc/run_r05a_reference_trajectory_lift_workload.sh; do
  ...
  test "$actual" = "$expected" || { echo "TRL-00A source hash changed: $relative" >&2; exit 2; }
done

export TRL_SOURCE_CONTRACT=$SOURCE_CONTRACT
exec "$WORKLOAD"
