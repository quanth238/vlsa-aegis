...
: "${SLURM_JOB_ID:?AF-00A publisher must run inside Slurm}"
: "${SLURM_JOB_PARTITION:?AF-00A publisher requires partition provenance}"
: "${SOURCE_JOB_ID:?exact AF-00A H100 job id is required}"
: "${SOURCE_CONTRACT:?AF-00A source contract is required}"
: "${EXPECTED_SOURCE_CONTRACT_SHA256:?AF-00A source-contract digest is required}"
: "${HELD_GPU_SUBMISSION:?AF-00A held GPU receipt is required}"
: "${EXPECTED_HELD_GPU_SUBMISSION_SHA256:?AF-00A held GPU receipt digest is required}"
: "${SUBMISSION:?AF-00A atomic submission receipt is required}"
: "${RELEASE_FINGERPRINT:?AF-00A release fingerprint is required}"
: "${RELEASE_FINGERPRINT_SHA256_FILE:?AF-00A release fingerprint digest file is required}"
...
fallback_receipt() {
  ...
  if [ "$status" -ne 0 ] && [ ! -f "$RECEIPT" ]; then
    ...
    jq -n --arg publisher "$SLURM_JOB_ID" --arg source "$SOURCE_JOB_ID" \
      --arg state "$SOURCE_STATE" --arg exit_code "$SOURCE_EXIT" --arg stage "$FAILURE_STAGE" \
      --arg result "$RESULT" --arg wrapper_status "$status" \
      '{schema_version:"1.0",artifact_role:"r05a_actual_forward_canary_cpu_publication",publisher_job_id:$publisher,source_job_id:$source,source_task_id:($source+"_0"),source_job_state:(if $state=="" then null else $state end),source_exit_code:(if $exit_code=="" then null else $exit_code end),failure_stage:$stage,result_path:$result,result_sha256:null,passed:false,published:false,errors:["publisher wrapper failed with exit code "+$wrapper_status],scientific_claim_allowed:false,simulator_efficacy_claim_allowed:false,infeasibility_claim_allowed:false,probe_training_authorized:false,automatic_next_gate_authorized:false}' >"$temporary" && mv "$temporary" "$RECEIPT"
  fi
  return "$status"
}
...
test "$SLURM_JOB_PARTITION" = main || { echo "AF-00A publisher is frozen to main" >&2; exit 2; }
test "${SLURM_CPUS_PER_TASK:-}" = 2 || { echo "AF-00A publisher requires two CPUs" >&2; exit 2; }
test "${SLURM_MEM_PER_NODE:-0}" = 8192 || { echo "AF-00A publisher requires exactly 8 GiB" >&2; exit 2; }
test "${CUDA_VISIBLE_DEVICES:-NoDevFiles}" = NoDevFiles || { echo "AF-00A publisher must not receive a GPU" >&2; exit 2; }
case "$SOURCE_JOB_ID" in *[!0-9]*|'') echo "invalid AF-00A source job id" >&2; exit 2 ;; esac
test "$SOURCE_CONTRACT" = "$RUN_ROOT/source-contract.json" || { echo "AF-00A source-contract path changed" >&2; exit 2; }
test "$HELD_GPU_SUBMISSION" = "$RUN_ROOT/held-gpu-submission.json" || { echo "AF-00A held GPU receipt path changed" >&2; exit 2; }
test "$SUBMISSION" = "$RUN_ROOT/submission.json" || { echo "AF-00A atomic submission path changed" >&2; exit 2; }
...
for path in "$CONFIG" "$LEGACY_CONFIG" "$SOURCE_CONTRACT" "$HELD_GPU_SUBMISSION" "$SUBMISSION" "$RELEASE_FINGERPRINT" "$RELEASE_FINGERPRINT_SHA256_FILE" "$STATUS_HELPER" "$RUNTIME_IDENTITY_HELPER" "$REMOTE_REPO/main/publish_crfs_r05a_actual_forward_canary.py"; do
  test -f "$path" && test ! -L "$path" || { echo "missing or symlinked AF-00A publisher input: $path" >&2; exit 2; }
done
. "$RUNTIME_IDENTITY_HELPER"
test "$LIBERO_PYTHON" = "$CRFS_R05A_LIBERO_PYTHON" || { echo "noncanonical AF-00A publisher launcher" >&2; exit 2; }
crfs_validate_r05a_libero_python "$LIBERO_PYTHON"
test "$(sha256sum "$SOURCE_CONTRACT" | awk '{print $1}')" = "$EXPECTED_SOURCE_CONTRACT_SHA256" || { echo "AF-00A source contract changed" >&2; exit 2; }
...
case "$EXPECTED_SUBMISSION_SHA256" in *[!0-9a-f]*|'') echo "invalid AF-00A submission digest" >&2; exit 2 ;; esac
test "${#EXPECTED_SUBMISSION_SHA256}" = 64 || { echo "invalid AF-00A submission digest length" >&2; exit 2; }
test "$(sha256sum "$SUBMISSION" | awk '{print $1}')" = "$EXPECTED_SUBMISSION_SHA256" || { echo "AF-00A atomic submission changed" >&2; exit 2; }
test "$(git -C "$REMOTE_REPO" rev-parse HEAD)" = "$EXPECTED_GIT_COMMIT" || { echo "AF-00A publisher commit changed" >&2; exit 2; }
test -z "$(git -C "$REMOTE_REPO" status --porcelain)" || { echo "AF-00A publisher tree is dirty" >&2; exit 2; }
...
' "$CONFIG" >/dev/null || { echo "AF-00A publisher release changed" >&2; exit 2; }
test "$(git -C "$REMOTE_REPO" rev-list --parents -n 1 "$EXPECTED_GIT_COMMIT")" = "$EXPECTED_GIT_COMMIT $ACCEPTED_IMPLEMENTATION_COMMIT" || { echo "AF-00A publisher release parent changed" >&2; exit 2; }
...
' "$SOURCE_CONTRACT" >/dev/null || { echo "AF-00A publisher source binding changed" >&2; exit 2; }
...
jq -e --arg run "$RUN_ID" --arg commit "$EXPECTED_GIT_COMMIT" --arg gpu "$SOURCE_JOB_ID" --arg cpu "$SLURM_JOB_ID" \
  ...
  ' "$RELEASE_FINGERPRINT" >/dev/null || { echo "AF-00A publisher release fingerprint changed" >&2; exit 2; }
jq -e --arg run "$RUN_ID" --arg commit "$EXPECTED_GIT_COMMIT" --arg gpu "$SOURCE_JOB_ID" --arg cpu "$SLURM_JOB_ID" \
  --arg source "$SOURCE_CONTRACT" --arg source_sha "$EXPECTED_SOURCE_CONTRACT_SHA256" \
  --arg held "$HELD_GPU_SUBMISSION" --arg held_sha "$EXPECTED_HELD_GPU_SUBMISSION_SHA256" \
  ...
  ' "$SUBMISSION" >/dev/null || { echo "AF-00A publisher atomic submission changed" >&2; exit 2; }
while IFS= read -r key; do
  ...
  test -f "$path" && test ! -L "$path" || { echo "AF-00A publisher fingerprint binding is missing: $key" >&2; exit 2; }
  test "$(sha256sum "$path" | awk '{print $1}')" = "$digest" || { echo "AF-00A publisher fingerprint binding changed: $key" >&2; exit 2; }
done <<'EOF'
...
for field in "JobId=$SLURM_JOB_ID" JobState=RUNNING Partition=main Account=normal QOS=normal TimeLimit=00:15:00 Requeue=0 NumNodes=1 NumCPUs=2 CPUs/Task=2; do
  case " $publisher_job " in *" $field "*) ;; *) echo "AF-00A publisher field changed: $field" >&2; exit 2 ;; esac
done
publisher_req_tres=$(printf '%s\n' "$publisher_job" | sed -n 's/.* ReqTRES=\([^ ]*\).*/\1/p')
test "$(printf '%s\n' "$publisher_req_tres" | tr ',' '\n' | awk -F= 'NF && $1!="billing" {print $1}' | LC_ALL=C sort)" = "$(printf '%s\n' cpu mem node | LC_ALL=C sort)" || { echo "AF-00A publisher ReqTRES key set changed" >&2; exit 2; }
test "$(printf '%s\n' "$publisher_req_tres" | tr ',' '\n' | awk -F= '$1=="cpu" {print $2}')" = 2 || { echo "AF-00A publisher CPU request changed" >&2; exit 2; }
case "$(printf '%s\n' "$publisher_req_tres" | tr ',' '\n' | awk -F= '$1=="mem" {print $2}')" in 8G|8192M) ;; *) echo "AF-00A publisher memory request changed" >&2; exit 2 ;; esac
test "$(printf '%s\n' "$publisher_req_tres" | tr ',' '\n' | awk -F= '$1=="node" {print $2}')" = 1 || { echo "AF-00A publisher node request changed" >&2; exit 2; }
if printf '%s\n' "$publisher_req_tres" | tr ',' '\n' | awk -F= '$1 ~ /^gres\/gpu/ {found=1} END {exit !found}'; then
  echo "AF-00A publisher requests a GPU" >&2; exit 2
fi
case " $publisher_job " in *" TRESPerNode=gres/gpu:"*) echo "AF-00A publisher has per-node GPU resources" >&2; exit 2 ;; esac
...
test "$source_status" -eq 0 && test "$SOURCE_STATE" = COMPLETED && test "$SOURCE_EXIT" = 0:0 || {
  echo "AF-00A source task failed: state=${SOURCE_STATE:-missing} exit=${SOURCE_EXIT:-missing}" >&2; exit 3
}
...
test -f "$RESULT" && test -f "$RECEIPT" || { echo "AF-00A publisher omitted final artifacts" >&2; exit 4; }
