...
HISTORICAL_ADR0047_LOCAL=docs/decisions/0047-preserve-cfs00a-run-b-and-record-failed-jacobian-diagnostics.md
ADR0048_LOCAL=docs/decisions/0048-preserve-fd-diagnostic-and-normalize-terminal-transport.md
...
FD_DIAGNOSTIC_A_EVIDENCE_LOCAL=evidence/r05a/cfs00a-fd-diagnostic-20260716a.json
FD_DIAGNOSTIC_A_EVIDENCE_TEST_LOCAL=tests/test_r05a_cfs00a_fd_diagnostic_a_evidence.py
...
test "$(shasum -a 256 "$HISTORICAL_ADR0047_LOCAL" | awk '{print $1}')" = b4b62725b78156a095f7e6c83bbbfed7ac536ce820a381dad1931f4d7eb8510b || { echo "historical ADR-0047 changed" >&2; exit 2; }
test "$(shasum -a 256 "$FD_DIAGNOSTIC_A_EVIDENCE_LOCAL" | awk '{print $1}')" = 12cea62c3eca6ce1bc44d4af936a09ddccae77f1adb32f4b3776d999778adf69 || { echo "diagnostic-A evidence changed" >&2; exit 2; }
test "$(shasum -a 256 openpi/src/openpi/serving/websocket_policy_server.py | awk '{print $1}')" = 1370d345e6c3c5b8f15573050e485e60a5b423d1df33e24b237805e6b442b026 || { echo "WebSocket policy server changed" >&2; exit 2; }
test "$(shasum -a 256 openpi/packages/openpi-client/src/openpi_client/msgpack_numpy.py | awk '{print $1}')" = c04568948fcee52b691e3be4b6cffb759f7e79ad67530fcd5d23095a0d13c057 || { echo "msgpack transport changed" >&2; exit 2; }
test "$(shasum -a 256 openpi/packages/openpi-client/src/openpi_client/websocket_client_policy.py | awk '{print $1}')" = f96009f787d6ccdebde077cc4f11829b89fcce677532af44b3d206a93d53d5b4 || { echo "WebSocket client changed" >&2; exit 2; }
...
jq -e '
  ...
  and .release_commit == "3d44b2c5a4779725101d672ed29658a841c85541"
  and .source_host == "worker-1"
  and .jobs.gpu_array_task.job_id == "28212_0"
  and .jobs.cpu_afterany_validator.job_id == "28213"
  and .immutable_artifacts.published_result_sha256 == "5c843ce5ed4f0e836b43dd6e0b42fec3ae120fa61cb1c56350eac046001062e0"
  and .immutable_artifacts.repository_binding_count == 60
  and .execution_boundary.adapter_terminal_outer_keys == ["__crfs_terminal__"]
  and .execution_boundary.transport_outer_keys_received_by_client == ["__crfs_terminal__","server_timing"]
  and .execution_boundary.client_terminal_validation_completed == false
  and .execution_boundary.numeric_jacobian_persisted == false
  and .execution_boundary.finite_difference_numeric_comparisons_persisted == false
  and .execution_boundary.arm_b_fista_started == false
  and .execution_boundary.arm_b_candidate_exists == false
  and .execution_boundary.arm_c_executed == false
  ...
  and .failure.root_message == "terminal constrained-flow response must contain only its reserved key"
  and .operational_validation.allocation_tests_expected == 99
  and .operational_validation.allocation_tests_observed == 99
  and .operational_validation.allocation_test_skips == 0
  and .operational_validation.memory_event_deltas == {max:0,oom:0,oom_kill:0}
  and .interpretation.status == "apparatus_inconclusive"
  and .interpretation.automatic_h100_resubmission_authorized == false
  and .interpretation.probe_or_mlp_training_authorized == false' "$FD_DIAGNOSTIC_A_EVIDENCE_LOCAL" >/dev/null || { echo "diagnostic-A evidence semantics changed" >&2; exit 2; }
...
jq -e --arg run "$RUN_ID" --arg implementation "$ACCEPTED_IMPLEMENTATION_COMMIT" '
  ...
  and .execution_release.decision_artifact == "docs/decisions/0048-preserve-fd-diagnostic-and-normalize-terminal-transport.md"
  ...
  and .execution_release.allowed_release_diff_paths == ["configs/experiments/r05a_constrained_flow_canary.json","configs/experiments/r05a_constrained_flow_canary_apparatus.json","docs/decisions/0048-preserve-fd-diagnostic-and-normalize-terminal-transport.md"]
...
jq -e '.release_decision_artifact == "docs/decisions/0048-preserve-fd-diagnostic-and-normalize-terminal-transport.md"
  and .fd_diagnostic_a_evidence_binding == {
    ...
    gpu_task_id:"28212_0",cpu_publisher_job_id:"28213",
    published_result_sha256:"5c843ce5ed4f0e836b43dd6e0b42fec3ae120fa61cb1c56350eac046001062e0",
    classification:"apparatus_inconclusive",client_terminal_validation_completed:false,
    numeric_diagnostics_persisted:false,h100_submission_authorized_by_evidence:false
  }' "$APPARATUS_CONFIG_LOCAL" >/dev/null || { echo "apparatus diagnostic-A evidence binding changed" >&2; exit 2; }
...
test "$RELEASE_DIFF" = "$(printf '%s\n' "$CFS_CONFIG_LOCAL" "$APPARATUS_CONFIG_LOCAL" "$ADR0048_LOCAL")" || { echo "release-only path set changed" >&2; exit 2; }
...
git show "$ACCEPTED_IMPLEMENTATION_COMMIT:$ADR0048_LOCAL" >"$EXPECTED_RELEASE_DECISION"
test "$(shasum -a 256 "$EXPECTED_RELEASE_DECISION" | awk '{print $1}')" = 13f209c205f9121ecc0b871dba0f6c786fe4ad844d41fe311149f089c6decddd || { echo "accepted implementation ADR-0048 changed" >&2; exit 2; }
{
...
cmp -s "$EXPECTED_RELEASE_DECISION" "$ADR0048_LOCAL" || { echo "release decision is not canonical appendix" >&2; exit 2; }
...
HISTORICAL_ADR0047_SHA256=$(shasum -a 256 "$HISTORICAL_ADR0047_LOCAL" | awk '{print $1}')
ADR0048_SHA256=$(shasum -a 256 "$ADR0048_LOCAL" | awk '{print $1}')
...
FD_DIAGNOSTIC_A_EVIDENCE_SHA256=$(shasum -a 256 "$FD_DIAGNOSTIC_A_EVIDENCE_LOCAL" | awk '{print $1}')
...
ssh "$HOST" bash -s -- \
  ...
  "$APPARATUS_CONFIG_SHA256" "$ENVELOPE_SCHEMA_SHA256" "$ADR0040_SHA256" "$HISTORICAL_ADR0041_SHA256" "$ADR0045_SHA256" "$HISTORICAL_ADR0047_SHA256" "$ADR0048_SHA256" \
  "$RUNTIME_IDENTITY_EVIDENCE_SHA256" "$RUNTIME_IDENTITY_PREFLIGHT_SHA256" "$RUN_B_EVIDENCE_SHA256" "$FD_DIAGNOSTIC_A_EVIDENCE_SHA256" "$ACCEPTED_IMPLEMENTATION_COMMIT" \
...
expected_apparatus_sha=${12}; expected_schema_sha=${13}; expected_adr0040_sha=${14}; expected_historical_adr0041_sha=${15}; expected_adr0045_sha=${16}; expected_historical_adr0047_sha=${17}; expected_adr0048_sha=${18}
expected_runtime_identity_evidence_sha=${19}; expected_runtime_identity_preflight_sha=${20}; expected_run_b_evidence_sha=${21}; expected_fd_diagnostic_a_evidence_sha=${22}; accepted_implementation_commit=${23}
fresh_preflight_sha=${24}; fresh_preflight_base64=${25}
...
historical_adr0047=$remote_repo/docs/decisions/0047-preserve-cfs00a-run-b-and-record-failed-jacobian-diagnostics.md
adr0048=$remote_repo/docs/decisions/0048-preserve-fd-diagnostic-and-normalize-terminal-transport.md
...
fd_diagnostic_a_evidence=$remote_repo/evidence/r05a/cfs00a-fd-diagnostic-20260716a.json
...
bound_paths=(
  ...
  docs/decisions/0045-accept-runtime-identity-regression.md docs/decisions/0047-preserve-cfs00a-run-b-and-record-failed-jacobian-diagnostics.md docs/decisions/0048-preserve-fd-diagnostic-and-normalize-terminal-transport.md
  evidence/r05a/runtime-identity-regression-20260716a.json evidence/r05a/runtime-identity-preflight-20260715T221453Z.txt evidence/r05a/cfs00a-same-budget-launch-b.json evidence/r05a/cfs00a-fd-diagnostic-20260716a.json
  ...
  openpi/scripts/serve_cfs_policy.py openpi/scripts/serve_policy.py openpi/packages/openpi-client/src/openpi_client/msgpack_numpy.py openpi/packages/openpi-client/src/openpi_client/websocket_client_policy.py openpi/src/openpi/models_pytorch/crfs_inverse_control.py openpi/src/openpi/models_pytorch/crfs_linearized_control.py
  openpi/src/openpi/models_pytorch/pi0_pytorch.py
  ...
  openpi/src/openpi/policies/crfs_constrained_flow_adapter.py openpi/src/openpi/policies/policy.py openpi/src/openpi/serving/websocket_policy_server.py
  ...
  tests/test_linearized_flow_control.py tests/test_constrained_flow_adapter.py tests/test_r05a_constrained_flow_canary.py tests/test_r05a_constrained_flow_hpc_contract.py tests/test_r05a_constrained_flow_publication.py tests/test_r05a_constrained_flow_validation.py tests/test_r05a_runtime_identity_regression_evidence.py tests/test_r05a_cfs00a_launch_b_evidence.py tests/test_r05a_cfs00a_fd_diagnostic_a_evidence.py
)
...
test "$(sha256sum "$historical_adr0047"|awk '{print $1}')" = "$expected_historical_adr0047_sha"
test "$(sha256sum "$adr0048"|awk '{print $1}')" = "$expected_adr0048_sha"
...
test "$(sha256sum "$fd_diagnostic_a_evidence"|awk '{print $1}')" = "$expected_fd_diagnostic_a_evidence_sha"
test "$(sha256sum "$remote_repo/openpi/src/openpi/serving/websocket_policy_server.py"|awk '{print $1}')" = 1370d345e6c3c5b8f15573050e485e60a5b423d1df33e24b237805e6b442b026
test "$(sha256sum "$remote_repo/openpi/packages/openpi-client/src/openpi_client/msgpack_numpy.py"|awk '{print $1}')" = c04568948fcee52b691e3be4b6cffb759f7e79ad67530fcd5d23095a0d13c057
test "$(sha256sum "$remote_repo/openpi/packages/openpi-client/src/openpi_client/websocket_client_policy.py"|awk '{print $1}')" = f96009f787d6ccdebde077cc4f11829b89fcce677532af44b3d206a93d53d5b4
...
jq -e '
  ...
  and .interpretation.status == "apparatus_inconclusive"
  and .interpretation.automatic_h100_resubmission_authorized == false
  and .interpretation.probe_or_mlp_training_authorized == false' "$fd_diagnostic_a_evidence" >/dev/null || { echo "remote diagnostic-A evidence changed" >&2; exit 2; }
...
frozen_bindings=$(jq -n \
  ...
  --arg historical_adr0047 "$expected_historical_adr0047_sha" \
  --arg adr0048 "$expected_adr0048_sha" \
  ...
  --arg fd_diagnostic_a_evidence "$expected_fd_diagnostic_a_evidence_sha" \
  '{
    ...
    historical_adr0047_sha256:$historical_adr0047,
    adr0048_sha256:$adr0048,
    ...
    fd_diagnostic_a_evidence_sha256:$fd_diagnostic_a_evidence,
    ...
    websocket_policy_server_sha256:"1370d345e6c3c5b8f15573050e485e60a5b423d1df33e24b237805e6b442b026",
    msgpack_numpy_sha256:"c04568948fcee52b691e3be4b6cffb759f7e79ad67530fcd5d23095a0d13c057",
    websocket_client_policy_sha256:"f96009f787d6ccdebde077cc4f11829b89fcce677532af44b3d206a93d53d5b4",
