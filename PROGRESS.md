...
# 2026-08-11: direct-horizon OSC displacement gate registered

The user authorizes one decisive continuation that rejects job `38240`'s
recursive increments. The new shared time-conditioned decoder predicts every
`Delta q_k=q_k-q0` directly from complete structured OSC state and the full
two-action Cartesian chunk. `Delta q_0=0` is architectural; there is no
per-step increment prediction or cumulative integration. Direct displacement,
finite-difference action sensitivity, and symmetric near-boundary signed
...
false-safes, support in every recoverable state, at least 90% reserved recall,
at most 2.671122 mm boundary error, both sensitivity cosines above 0.8, exact
q0, bounded terminal/overall error, and no horizon drift. It is evaluated
against the frozen job-38070 factorized and job-38240 cumulative models on the
same reserved actions. Failure stops before calibration, QP, or closed loop.
Protocol:
